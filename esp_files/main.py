...
def send_sensory_data(arg):
    global ultrasonic_flag, IMU_flag, ultrasonic
    while True:
        if ultrasonic_flag == True:
            distance = ultrasonic.distance_cm()
            print("u " + str(distance))
            ultrasonic_flag = False
        if IMU_flag == True:            
...
def main():
    ...
    timer_for_ultrasonic.init(freq= 20, mode=Timer.PERIODIC, callback =lambda t: getdist())
    timer_for_IMU.init(freq=10, mode=Timer.PERIODIC, callback =lambda t: IMUprint())
  #  timer_for_ADC.init(freq=1, mode=Timer.PERIODIC, callback =lambda t: ADCprint())
   # timer_for_BUMPER.init(freq=1, mode=Timer.PERIODIC, callback =lambda t: BUMPERprint())
    ...
    _thread.start_new_thread(send_sensory_data, (5,))
